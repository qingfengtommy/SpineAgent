...
deepspeed --master_port 12540 --num_gpus 4 train/train_mem.py \
    ...
    --bf16 True \
    --fp16 False \
    --pretrain_mm_mlp_adapter /path/to/pretrained_mm_projector/mm_projector.bin \
    --output_dir ./spinal_checkpoints/agent/llava-$MODEL_VERSION-finetune-with-correction_epoch_$EPOCH \
    ...
    --report_to wandb  \
    --pretrain_mode False \
    --patient_id_list /path/to/pretrain_intersection_patient_ids.txt
