...
"""
    This class define the custom dataset for pretraining.
    Change it to accustom to your dataset.
"""

class NewDataset(Dataset):
    def __init__(self, base_dir, in_file, transform, modality='t1'):
        print(f'Loading data from {in_file} for modality {modality}.')
        images = []
        # 43922 patients for pretraining
        with open(in_file) as fin:
            data = json.load(fin)
            print(f"Total {len(data)} patients for pretraining.")
            for value in data.values():
                images.extend(value[modality])
...
For python-based LazyConfig, use "path.key=value".
    ...
    parser.add_argument("--modality", default="t1", type=str, choices=["t1", "t2"], help="image modality")
    return parser
...
def build_data_loader_from_cfg(
    ...
    dataset = NewDataset(
        base_dir = None,
        in_file="/home/gongbsun/dinov2_spinal/prepro/image_data_full_paths_final_clean.json",
        transform=model.build_data_augmentation_dino(cfg),
